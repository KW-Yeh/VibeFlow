...
```bash
~/.Codex/scripts/pr-context.sh [ticket | PR# | #PR]
```
...
PR 標題與 `## What`／`## Why`／`## How` 等其餘區塊維持英文，但在整個 PR body 的**最後**（對應連結之後、結尾的 `🤖 Generated with Codex` 之前），額外加入一個 `## 中文摘要` 區塊，用**繁體中文**重新描述這次更動。其上方需加一條 `---` divider 與英文內容區隔。規則：
...
- `## 中文摘要` 放在整個 PR body 的最後（對應連結之後、結尾 `🤖 Generated with Codex` 之前），上方加一條 `---` divider；以 high-level、主題式的繁體中文重新描述本次更動（所有類型 PR 皆必填），規則見上方「`## 中文摘要` 區塊」說明；不逐條翻譯 `## How`，每點以 1 句為原則
...
#### 6a. 準備
- PR body 結尾需加上 `🤖 Generated with [Codex](https://Codex.com/Codex)`
...
**建立新 PR 模式：**
```bash
~/.Codex/scripts/pr-create.sh --title "<標題>" --body-file <body-file> --base <base>
```
（`<base>` 用步驟 1 bundle 的 `base` 欄位；省略 `--base` 則自動用 repo 預設分支。）

**更新既有 PR 模式：**
```bash
~/.Codex/scripts/pr-create.sh --title "<標題>" --body-file <body-file> --update <PR#>
```
