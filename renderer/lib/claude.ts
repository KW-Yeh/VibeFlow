...
function buildProgressProtocolLines(
  ...
  return [
    ...
    `9a. 要給使用者看的驗證證據（UI 驗證截圖、互動錄製 GIF、視覺比對報告）→ 放 ${artifactsDir}/ 根目錄。做 UI 相關驗證時務必把證據存在這裡，使用者會在 VibeFlow 的「Artifacts」分頁直接檢視（GIF 會自動播放），不必自行開 dev server。這一區請保持精簡：只放你會主動請使用者過目的檔案。`,
    ...
    `9c. 錄製步驟：\`start_recording\` → 立刻截一張圖當首幀 → 執行互動 → 再截一張當末幀 → \`stop_recording\` → \`export\`（帶 \`download: true\` 與具描述性的 \`filename\`）。一段只涵蓋一條互動路徑，長度控制在 15 秒內；多條路徑分成多個檔，不要串成一長段。GIF 會以 base64 過 IPC 進 Artifacts 分頁，過大的檔會拖慢 UI。`,
